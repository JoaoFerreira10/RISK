...
public class AgenteAtacante extends AgenteRisk {
	...
	public String selecionarAtaque() {
		...
		for (int i = 0; i < tabuleiro.getTerritoriosPorAgente(this.getCor()).size(); i++) {
			...
			int x=0;
/*			for(int j1=0;j1<tabuleiro.getTerritoriosPorAgente(this.getCor()).size();j1++)
			{
				System.out.println("en");
				if(tabuleiro.getTerritorio(tabuleiro.getTerritoriosPorAgente(this.getCor()).get(j1)).getpecas() >= 
						tabuleiro.getTerritorio(tabuleiro.getTerritoriosPorAgente(this.getCor()).get(z)).getpecas()){
					System.out.println("entrou");
					z = tabuleiro.getTerritoriosPorAgente(this.getCor()).get(j1); 
					break;
				}
